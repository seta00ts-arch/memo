...
export default function SettingsPanel() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      setSyncStatus("done");
      setSyncMessage(
        `アップロード ノート${result.uploadedNotes}件/履歴${result.uploadedHistory}件/添付${result.uploadedAttachments}件、` +
          `ダウンロード ノート${result.downloadedNotes}件/添付${result.downloadedAttachments}件` +
